...
class ADDIDataModule(pl.LightningDataModule):

    def __init__(self, data_dir: str = './data/addi/', batch_size: int = 32, separate_multiple_annotations=True):
        """Data module for the ADDI dataset

        Args:
            data_dir (str, optional): the addi data directory. Defaults to './data/addi/'.
    ...
    def prepare_data(self):
        ...
        if not os.path.isdir(os.path.join(self.data_dir, "train")):
            ...
            from data.addi.download import main as download_addi
            download_addi()
...
if __name__ == '__main__':
    dm = ADDIDataModule(separate_multiple_annotations=False)
